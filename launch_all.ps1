...
#}
Start-Process powershell.exe -ArgumentList '-NoExit', '-Command', 'cd backend; uv run uvicorn src.api.main:app --reload --host 0.0.0.0 --port 8000'
Start-Process powershell.exe -ArgumentList '-NoExit', '-Command', 'cd frontend; uv run streamlit run app.py --server.port 8501'
