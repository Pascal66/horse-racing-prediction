...
class BacktestService:
    ...
    def run_backtest(self) -> Dict[str, Any]:
            ...
            for race_id, group in model_df.groupby('race_id'):
                ...
                if len(top3_win) == 3:
                    ...
                    if (top3_win['finish_rank'].between(1, 3)).all():
                        trio_rows = group[group['bet_type'].str.contains('TRIO', na=False) & ~group['bet_type'].str.contains('ORDRE', na=False)]
        ...
        for _, row in unique_contexts.iterrows():
            best_algos[(row['discipline'], row['month'])] = self.repository.get_best_model_for_context(row['discipline'], row['month'])
        ...
        for race_id, race_group in df.groupby('race_id'):
            ...
            is_win = int(best_horse['finish_rank']) == 1
            win_div = model_group[(model_group['bet_type'] == 'E_SIMPLE_GAGNANT') & (model_group['combination'] == str(best_horse['program_number']))]['dividend_per_1e']
            payout_sg = float(win_div.iloc[0]) if not win_div.empty else (float(best_horse['effective_odds']) if is_win else 0.0)
            ...
            if len(top3_win) == 3:
                nums = set(map(int, top3_win['program_number']))
                if (top3_win['finish_rank'].between(1, 3)).all():
                    trio_rows = model_group[model_group['bet_type'].str.contains('TRIO', na=False) & ~model_group['bet_type'].str.contains('ORDRE', na=False)]
                    for _, row in trio_rows.iterrows():
                        try:
                            if set(map(int, str(row['combination']).replace('-', ' ').split())) == nums:
                                payout_trio = float(row['dividend_per_1e'])
