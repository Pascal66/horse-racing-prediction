...
class BacktestService:
    ...
    def calculate_roi_for_df(self, df: pd.DataFrame, race_divs: dict, min_bets: int = 1) -> Dict[str, Any]:
        ...
        for model, data in results.items():
            ...
            for bt in ['SG', 'SP', 'CG', 'CP', 'TRIO']:
                ...
                if bt == 'SG':
                    data['profit'] = profit
                    data['roi'] = self._safe_float((tr - tm) / tm * 100) if tm > 0 else -100.0
                    data['count'] = int(tm)
                    data['nb_wins'] = int(tw)
                else:
                    suffix = f"_{bt.lower()}"
                    ...
                    data[f"nb_bets{suffix}"] = int(tm)
                    data[f"nb_wins{suffix}"] = int(tw)

            data["max_daily_profit"] = max_profit
            # Win Rate global basé sur le nombre de courses gagnées (SG)
            data["win_rate"] = self._safe_float(data["nb_wins"] / (data["count"] / 3) * 100) if data["count"] > 0 else 0
    ...
    def get_daily_stats_from_db(self, date: datetime.date) -> Dict[str, Any]:
        ...
        for r in rows:
            ...
            bt = r['bet_type'].upper()
            pk, rk, nk, wk = (f"profit_{bt.lower()}" if bt != 'SG' else "profit"), (
                f"roi_{bt.lower()}" if bt != 'SG' else "roi"), (f"nb_bets_{bt.lower()}" if bt != 'SG' else "count"), (
                f"nb_wins_{bt.lower()}" if bt != 'SG' else "nb_wins")
    ...
    def _index_divs(self, df: pd.DataFrame) -> dict:
        ...
        for rid, group in df.groupby('race_id'):
            ...
            for _, row in group[pd.to_numeric(group.get('dividend_per_1e', 0), errors='coerce') > 0].iterrows():
                ...
                k = 'SG' if 'SIMPLE_GAGNANT' in bt else 'SP' if 'SIMPLE_PLACE' in bt else 'CG' if 'COUPLE_GAGNANT' in bt else 'CP' if 'COUPLE_PLACE' in bt else 'TRIO' if (
                            'TRIO' in bt and 'ORDRE' not in bt) else None
                if k: d_map.setdefault(k, []).append(
                    (self._parse_comb(row['combination']), float(row['dividend_per_1e'])))
            race_divs[rid] = d_map
        return race_divs
