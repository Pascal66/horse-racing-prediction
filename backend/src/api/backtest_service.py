    ...
    def run_full_backtest(self, date_start: Optional[str] = None, date_end: Optional[str] = None) -> Dict[str, Any]:
        ...
        for model in df['model_version'].unique():
            ...
            for race_id, group in model_df.groupby('race_id'):
                ...
                win_div = \
                group[(group['bet_type'] == 'SG') & (group['combination'] == str(best_horse['program_number']))][
                    'dividend_per_1e']
        ...
        for race_id, group in sniper_bets_df.groupby('race_id'):
            ...
            win_div = group[(group['bet_type'] == 'SG') & (group['combination'] == str(best['program_number']))][
                'dividend_per_1e']
                ...
                for race_id, allocation in kelly_report.get('bankroll_allocation', {}).items():
                    ...
                    for prog_num, frac in fractions.items():
                        ...
                        if not horse.empty:
                            ...
                            # Get actual dividend
                            win_div = day_df[(day_df['race_id'] == race_id) & (day_df['bet_type'] == 'SG') & (
                                        day_df['combination'] == str(prog_num))]['dividend_per_1e']
                            payout_per_1e = win_div.iloc[0] if not win_div.empty else (
                                horse['effective_odds'] if is_win else 0.0)
