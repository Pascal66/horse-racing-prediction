...
class PerformancesIngestor(BaseIngestor):
    ...
    def _preload_horse_cache(self):
        """Pre-loads existing horses map (Name -> ID) to speed up lookups."""
    ...
    def _fetch_perf_json(self, session, meeting, race):
        """Fetches performance JSON data from the external API."""
    ...
    def _get_horse_id_thread_safe(self, horse_name):
        """Thread-safe retrieval or creation of a Horse ID."""
        if not horse_name:
            return None
    ...
    def _prepare_history_data(self, horse_id, history_item):
        """
        Parses a single historical race item into a tuple suitable for database insertion.
        Returns None if data is invalid or irrelevant.
        """
        if not horse_id:
            return None
    ...
    def _process_single_race(self, meeting_num, race_num):
        """Worker method to process performances for a single race."""
            ...
            with conn:
                with conn.cursor() as cursor:
                    ...
                    if batch_values:
                        ...
                        """
                        psycopg2.extras.execute_values(cursor, query, batch_values)
